...
namespace WpfWindowsLib {
  ...
  public class EmailTextBox: CheckedTextBox {
    ...
    protected override void OnPreviewKeyDown(KeyEventArgs e) {
      if (!EmailValidator.IsBlankAllowed) {
    ...
    protected override void OnPreviewTextInput(TextCompositionEventArgs e) {
      ...
      if (e.Text.Length>0) { //ctrl + key results in Text.Length==0
        if (e.Text.Length>1 || e.Text[0]>0x7F) {
          ...
          isError = !EmailValidator.IsInternationalCharSetAllowed;
        } else {
          ...
          if (checkChar=='@') {
            ...
            if (atIndex<0 || CaretIndex<=atIndex) {
              isError = !EmailValidator.IsValidEmailChar(checkChar);
            } else {
              isError = !EmailValidator.IsValidDnsChar(checkChar);
            }
          }
        }
      }
